...
`StaticMeshEditor` composes its left side through `StaticMeshToolDock`: a collapsible rail containing the existing `MeshAssetHierarchy` plus Static Mesh sculpt/deformation controls. The dock is a UI host only and writes the same `EditorContext` / `meshEditing.configureSoftSelection(...)` state used by Scene and Tool Options. Loop selection is routed into the editor's local `AssetViewportEngine.selectionSystem`; topology traversal remains centralized in `SelectionSystem`/`MeshTopologyUtils`. See [`STATIC_MESH_WORKSPACE_DOCK.md`](./STATIC_MESH_WORKSPACE_DOCK.md).

## Editor Command Catalogue / capability-driven tool reuse

Modeling/editor commands are registered in `editor/commands/EditorCommandRegistry.ts`; Static Mesh built-ins live in `editor/commands/StaticMeshCommandCatalogue.ts`. UI surfaces such as Pie Menu and `StaticMeshToolDock` resolve command metadata and enabled state from this catalogue rather than owning separate callbacks. Scene and asset editors provide context adapters/capabilities, while the command ID stays stable. Editable mesh ownership is resolved through `engine/mesh-editing/StaticMeshEditTarget.ts`; Scene enables Static Mesh commands only for a single entity referencing a `MESH` asset, and Skeletal Mesh requires an explicit future source-mesh edit capability. See [`EDITOR_COMMAND_CATALOGUE.md`](./EDITOR_COMMAND_CATALOGUE.md).
