...
## Multi-Engine & Asset Viewport Gizmo Architecture

### 1. Engine Decoupling via EngineAPI
To support transforming objects within secondary viewports (e.g., `StaticMeshEditor`, `SkeletonEditor`) that run their own localized `AssetViewportEngine` instances:
- `GizmoSystem` is decoupled from the global `engineInstance` singleton. It accepts any `IEngine` implementing the unified `api: EngineAPI` contract.
- When dragging an axis or plane handle, `GizmoSystem` delegates mutations through `engine.api.commands.transform.setWorldPosition(entityId, newPos)`.
- Updates automatically dispatch to the target engine's ECS storage, mark the scene graph dirty via `sceneGraph.setDirty(entityId)`, compute local transforms via `syncTransforms(true)`, and notify listeners through `engine.notifyUI()`.

### 2. Built-in Asset Viewport Gizmo Handling
`AssetViewport3D` handles mouse interactions directly:
- On `mousedown`, `mousemove`, and `mouseup`, it queries `gizmoSystem.update(screenCoords, isDown, isUp)`.
- If the gizmo hit-tests an active handle (axis, plane, or center), the event is consumed by the gizmo, preventing unwanted raycast selection changes.
- Raycast tolerance and handle geometries are calibrated for reliable hit detection across all camera zoom levels.

### 3. ECS SoA Transform Flag Propagation
When entity transform proxies (`entity.components.Transform.position = ...` or `.x = ...`) or numeric inspector controls mutate coordinates:
- The proxy setters set both the coordinate array and `store.transformDirty[index] = 1`.
- `sceneGraph.setDirty(entityId)` propagates dirty flags down the scene hierarchy.
- This guarantees that both manual numeric typing, draggable step increments, and 3D gizmo manipulations update the object's world matrix synchronously on the next render pass.
