...
export const DraggableWindow = ({ 
    ...
    const velocityTracker = useRef({ lastX: 0, lastY: 0, lastTime: 0, vx: 0, vy: 0 });
    ...
    useEffect(() => {
        const handleMove = (e: MouseEvent) => {
            if (resizing && windowRef.current) {
                ...
                let newY = Math.max(0, Math.min(window.innerHeight - 30, e.clientY - dragOffset.current.y));
                
                const now = performance.now();
                const dt = now - velocityTracker.current.lastTime;
                
                if (dt > 0) {
                    const vx = (e.clientX - velocityTracker.current.lastX) / dt;
                    const vy = (e.clientY - velocityTracker.current.lastY) / dt;
                    
                    const VELOCITY_THRESHOLD = 2.0; // strong speed
                    
                    if (Math.abs(vx) > VELOCITY_THRESHOLD && Math.abs(vx) > Math.abs(vy)) {
                        if (vx < 0) {
                            newX = 0;
                        } else {
                            newX = window.innerWidth - (windowRef.current?.offsetWidth || size.w as number);
                        }
                        // Adjust offset so drag can continue without jumping back
                        dragOffset.current.x = e.clientX - newX;
                    } else if (Math.abs(vy) > VELOCITY_THRESHOLD && Math.abs(vy) > Math.abs(vx)) {
                        if (vy < 0) {
                            newY = 0;
                        } else {
                            newY = window.innerHeight - (windowRef.current?.offsetHeight || (typeof size.h === 'number' ? size.h : 400));
                        }
                        dragOffset.current.y = e.clientY - newY;
                    }
                }
