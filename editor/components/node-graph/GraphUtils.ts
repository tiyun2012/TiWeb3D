...
export const GraphUtils = {
    ...
    calculateCurve: (x1: number, y1: number, x2: number, y2: number) => {
        ...
        // If the target is behind the source, we add some vertical looping offset
        if (x1 > x2) {
            cX1 = x1 + offset + (x1 - x2) * 0.2;
            cX2 = x2 - offset - (x1 - x2) * 0.2;
        }

        return `M ${x1} ${y1} C ${cX1} ${y1} ${cX2} ${y2} ${x2} ${y2}`;
    },

    canConnect: (typeA: string, typeB: string) => {
        if (!typeA || !typeB) return true;
        if (typeA === typeB) return true;
        if (typeA === 'any' || typeB === 'any') return true;
        
        // Allow implicit conversions between math types
        const isMathA = ['float', 'vec2', 'vec3'].includes(typeA);
        const isMathB = ['float', 'vec2', 'vec3'].includes(typeB);
