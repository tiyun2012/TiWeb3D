...
export const NodeItem = memo(({ 
    ...
    const borderStyle = selected ? 'ring-1 ring-accent border-accent shadow-[0_0_20px_rgba(79,128,248,0.2)]' : 'border-white/10 shadow-xl';
    ...
    const renderPort = (pinId: string, type: 'input'|'output', color?: string, portType?: string) => {
        ...
        return (
            <div 
                className={`absolute w-3 h-3 rounded-full border-[1.5px] ${borderClass} ${scaleClass} transition-all cursor-crosshair z-10 shadow-sm`}
                style={{ backgroundColor: bgStyle, [type === 'input' ? 'left' : 'right']: -16, top: '50%', transform: 'translateY(-50%)' }}
    ...
    return (
        <div className={`flex flex-col w-full h-full pointer-events-auto transition-shadow ${isReroute ? '' : `rounded-lg bg-black/60 backdrop-blur-md border ${borderStyle}`}`}>
             {isReroute ? (
                ...
                <>
                    <div className={`px-3 flex items-center justify-between border-b border-white/10 rounded-t-lg cursor-grab active:cursor-grabbing ${selected ? 'bg-gradient-to-r from-accent/40 to-accent/10' : 'bg-gradient-to-r from-white/10 to-transparent'}`} style={{ height: LayoutConfig.HEADER_HEIGHT }} onMouseDown={(e) => onMouseDown(e, node)}>
                        <div className="flex items-center gap-2 pointer-events-none">
                             <Icon name={(def.category === 'Output' || def.category === 'Shader') ? 'Cpu' : 'Settings2'} size={12} className={selected ? 'text-white' : 'text-gray-400'} />
                             <span className={`text-[11px] font-bold uppercase tracking-wider ${selected ? 'text-white' : 'text-gray-200'}`}>{def.title}</span>
                        </div>
                    </div>
                    
                    <div className="flex-1 flex flex-col overflow-hidden" style={{ paddingTop: LayoutConfig.PADDING_TOP, paddingLeft: 10, paddingRight: 10, paddingBottom: 8 }}>
                            ...
                            return (
                                <div key={input.id} className="relative flex items-center justify-between" style={{ height: LayoutConfig.ITEM_HEIGHT }}>
                                    <div className="flex items-center">
                                        ...
                                        <span className={`text-[10px] ml-2 transition-colors ${isConnected ? 'text-white font-bold' : 'text-gray-400'}`}>{input.name}</span>
                                    </div>
                                        ...
                                        <div className="ml-4 flex-1 flex justify-end" style={{ height: 16 }}>
                                            {input.type === 'vec3' ? (
                                                ...
                                                <input 
                                                    ...
                                                    onChange={(e) => onDataChange(node.id, input.id, e.target.value)} 
                                                    className="w-12 bg-black/60 text-[10px] text-white px-1.5 rounded border border-white/10 h-4 outline-none focus:border-accent text-right font-mono" 
                                                    onMouseDown={e => e.stopPropagation()} 
                                                />
                                            )}
                                        </div>
                        ...
                        <div style={{ marginTop: LayoutConfig.OUTPUTS_OFFSET }}>
                            {def.outputs.map(output => (
                                <div key={output.id} className="relative flex items-center justify-end" style={{ height: LayoutConfig.ITEM_HEIGHT }}>
                                    <span className="text-[10px] text-gray-200 font-bold mr-2 tracking-wide">{output.name}</span>
