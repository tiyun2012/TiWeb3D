...
export class GizmoSystem {
    ...
    private getSelectedComponentCentroid(entityId: string): Vector3 {
        ...
        const vertexIds = Array.from(this.engine.selectionSystem.getSelectionAsVertices()); // Updated
    ...
    private handleDrag(ray: any, entityId: string, isComponentMode: boolean) {
        ...
        if (hit) {
            ...
            if (isComponentMode) {
                ...
                this.engine.updateVertexDrag(entityId, totalDelta);
            } else {
                const target = Vec3Utils.subtract(constrainedHit, this.clickOffset, {x:0,y:0,z:0});
                if ((this.engine as any).api?.commands?.transform?.setWorldPosition) {
                    (this.engine as any).api.commands.transform.setWorldPosition(entityId, target);
                } else {
